#!/bin/bash
# This is literally just ripped from bazzi's 340 test script, modified somewhat by yours truly

if [ ! -d "./extra_tests" ]; then
...
for test_file in $(find ./extra_tests -type f -name "*.txt" | sort); do
    all=$((all+1))
